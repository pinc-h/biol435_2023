...
example(a,b) = a + b
callme(example)
# Functions of the same name can be assigned different data types, Julia 
will know which function to call based on the data type given when called
...
s = @spawnat 2 1 .+ fetch(r) # '@' before something in Julia calls a 
native macro
fetch(s)
x = @async 1+2
# Will return a 'remoteref', a ticket to identify the process (a container 
value)
fetch(x)
r = RemoteRef() # Will artificially create a ticket without a process
@async (fetch(r);println("hi")) # Will not return anything, defines the 
process that will occur when a value gets put in this ticket
